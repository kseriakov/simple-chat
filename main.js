...
$(document).ready(function () {
  ...
  if ($(".newsmodule-slider").length > 0) {
    $(".newsmodule-slider").slick({
      ...
      //appendArrows:$('.mainslider-arrows .container'),
      nextArrow:
        '<button type="button" class="slick-next fa fa-angle-right"></button>',
      prevArrow:
        '<button type="button" class="slick-prev fa fa-angle-left"></button>',
      responsive: [
        {
          breakpoint: 768,
          settings: {},
    ...
    $.each($('.select[data-type="search"]'), function (index, val) {
      ...
      if ($(this).find(".select-options__value:visible").length == 1) {
        ...
        block
          ...
          .attr(
            "data-value",
            select.find('option[selected="selected"]').html()
          );
      }
    });
  }
